---
name: search-toolkit
description: Search Crew 的搜索 / 抓取 / 站点 API / 产物组织工具包。供 evidence-search / site-search / deep-search / wide-search 复用。零依赖 Python stdlib。
---
...
| 脚本 | 用途 | 关键参数 |
|---|---|---|
| `search.py` | 通用搜索 | `--query`、`--max-results`、`--language`、`--prefer jina\|serper\|bocha\|ai`、`--with-content`（jina 一次带回正文，省单独抓页） |
| `ai_search.py` | AI 综述快答（/search-fast 后端，主 agent 直连） | `--query`；按语言/语境选 AI 源，输出 `{backend,summary,citations}` |
| `fetch.py` | URL → markdown（多 URL 并发；直连/jina-reader，`--real-browser` 时必要场景升级 universal-page-fetcher，兜底 WebFetch） | `[--real-browser] <url> [<url>...]`；任何调用方按需开启升级（需登录态 / 虚拟滚动 / 被挡），批量只对确有需要的 URL 开 |
...
1. 文件 / 子目录前缀以 subagent 名开头：`evidence-search-*` / `site-search-*` / `deep-search-*`
